...
<div class="main">
  ...
  <div class="header">
	...
	<div class="menu_nav">
        <ul>   <li class="active"><a href="home.php"><span>Home Page</span></a></li> <li><a href="members.php"><span>Members</span></a></li>
...
<script type="text/javascript">
        function PrintDiv(id) {
            ...
            myWindow.document.write('<html><head><title>Kibabii University </title>');
	...
	<form id="form7">
 <button onclick="PrintDiv('print')">Print Statement</button>
</form>
	<div id="print">
<fieldset> <legend align="center">Fee Statement </legend>
<img src="images/kevo.png"style="float:left; padding:5px;" height="100" width="100" />
<table align="center"><tr><td>Reg No:</td><td><?php echo"$reg_no";?></td><td>Name:</td><td><?php echo"$f_name $lname";?></td><td>County:</td><td><?php echo"$user_county";?></td></tr>
</table>
<?php
...
$input = "insert into school_account (total,reg_no,credit,confirmation_id)
 values('$new_ttl','$reg_no','$credit','$code')";
...
$get_sem="select * from transaction where user_id='$user_id' and description!='' ";
$run_sem=mysqli_query($con,$get_sem);
...
$get_sem="select * from transaction where user_id='$user_id' ORDER by 1 DESC LIMIT 1";
$run_sem=mysqli_query($con,$get_sem);
...
$get_sem="select * from transaction where semester='$semester' and user_id='$user_id'";
$run_sem=mysqli_query($con,$get_sem);
$check=mysqli_num_rows($run_sem);
if($check>=1)
{
...
echo"<script>window.open('transcript.php','_self')</script>";
exit();
}
...
$amount=$_POST['amount'];
$get_user="select * from record where account_number='$account'";
...
{
$my_idd=$row['rec_id'];
$get_pass="select * from record where password='$password' and account_number='$account' ";
$run_pass=mysqli_query($conn,$get_pass);
$check4=mysqli_num_rows($run_pass);
if($check4==0){
echo"<script> alert('account or password incorect!')</script>";
exit();
}
if($check4==1)
{
//buying tokens
$my_id=$row['rec_id'];
$get_acci="select * from transaction where account_number='$account'  ORDER by 1 DESC LIMIT 1";
...
$rec=$balance-$amount;
$insert = "insert into transaction (user_id,receiver_id,account,account_number)
 values('$used','$used','$rec','$account')";
$run = mysqli_query($conn,$insert);
//getting school account
...
$school_account=$row['school_account'];
$get_account="select * from  record where account_number='$school_account'";
...
if($check==1)
{
$school_id=$row['rec_id'];

$get_acbl="select * from  transaction where account_number='$school_account' ORDER by 1 DESC LIMIT 1";
$run_acbl=mysqli_query($conn,$get_acbl);
$row=mysqli_fetch_array($run_acbl);
$accountcheck=$row['account'];
$newcheck=$accountcheck+$amount;
$insert = "insert into transaction (user_id,receiver_id,account_number,amount,account,from_account)
 values('$my_idd','$school_id','$school_account','$amount','$newcheck','$account')";
