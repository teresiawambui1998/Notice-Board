...
?>
	...
	<p>(1)Choose to pay by Pesa Pay(PP) Or use of Tertiary Finance Card(TF_Card) then click pay button to make payment.</p> </fieldset>


 
<form action="" method="Post" id="form3">
<fieldset><legend align="center"><H3>Pay School Feee Today.</H3></legend>

<table  align="center">
<tr> <td  id="hdtd"colspan="2" bgcolor="#99CCFF" >Pesa Pay(PP)</td></tr>
 <tr>
 
  <td width="50"><tr><td>Account</td>
    <td align="right"><input type="number" name="account" placeholder="your pesa pay account." required="required"></td>
     </tr>
	 <tr>
  <td width="50"><tr><td>Amount</td>
    <td align="right"><input type="number" name="amount" placeholder="enter amount." required="required"></td>
     </tr>
	  <tr>
  <td width="50"><tr><td>Password</td>
    <td align="right"><input type="password" name="password" placeholder="enter Pesa Pay password." required="required"></td>
     </tr>
	
    <tr>
 
    <td align="right" colspan="6">
	<button name="token" onclick='return buytoken()'>Pay</button>
   </table>
   </form>
...
$insert = "insert into  paybill_received (paybill_no,account_number,amount,original_balance,balance)
 ...
 $insert = "insert into  
 pay_bill_record (transaction_id,account_number,balance,amount,transaction_fee,paybill_balance,paybill_no,biller_id,original_balance)
 values('$code','$account1','$rec','$amount','$withdrawfee','$newcheck','$pay_bill_no','$rec_id','$balance')";
 $run = mysqli_query($conn,$insert);
 //updating school account
  $get_acbl="select * from   pesa_transactions ORDER by 1 DESC LIMIT 1";
$run_acbl=mysqli_query($con,$get_acbl);
$row=mysqli_fetch_array($run_acbl);
$total=$row['total'];
$new_total=$total+$amount;

  
 $insert = "insert into  pesa_transactions (reg_no,amount,total,trans_id)
 values('$reg_no','$amount','$new_total','$code')";
 $run = mysqli_query($con,$insert);
}
...
echo"<script>window.open('pesapay.php','_self')</script>";
exit();
}
